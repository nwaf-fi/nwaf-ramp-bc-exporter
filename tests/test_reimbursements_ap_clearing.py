...
def _make_reimb(id_: str, employee: str, payment_date: str, line_items: list,
    ...
    return {
        'id': id_,
        'user_full_name': employee,          # Ramp field for display name
        'payment_processed_at': f'{payment_date}T00:00:00Z' if payment_date else None,
        'transaction_date': transaction_date or payment_date,  # expense date
...
def test_single_reimbursement_five_line_items():
    ...
    clearing = df[df['Document No.'].str.endswith('-CLR')]
...
def test_two_reimbursements_same_employee_same_date():
    ...
    reimbs = [
        _make_reimb('R002', 'Bob Jones', '2026-02-10', items_a),
        _make_reimb('R003', 'Bob Jones', '2026-02-10', items_b),
    ]
    cfg = dict(BASE_CFG, target_month='2026-02')
    df = ramp_reimbursements_to_bc_rows(reimbs, cfg)
...
# ---------------------------------------------------------------------------
def test_missing_payment_date_skipped_when_period_active():
    items = [_make_line_item(5000, '63000')]
    reimbs = [
    ...
    cfg = dict(BASE_CFG, target_month='2026-02')
    df = ramp_reimbursements_to_bc_rows(reimbs, cfg)
    assert df.empty or 'REIMB-R009' not in df['Document No.'].values
