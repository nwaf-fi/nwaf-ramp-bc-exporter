                ...
                if st.checkbox('I confirm: mark these transactions as synced', value=False, key='cc_confirm_mark'):
                    if st.button('Mark these transactions as synced in Ramp', key='cc_mark_btn'):
                        with st.spinner('Marking transactions as synced...'):
                            ...
                            # Per Ramp API docs, only SYNC_READY transactions can be posted to
                            # /accounting/syncs. Transactions must be marked SYNC_READY in the
                            # Ramp UI by users — the API cannot set this status.
                            sync_ready_txns = [t for t in cc_cached if t.get('sync_status') == 'SYNC_READY']
                            not_ready_txns = [t for t in cc_cached if t.get('sync_status') != 'SYNC_READY']
                            total = len(cc_cached)

                            if not_ready_txns:
                                not_ready_statuses = {t.get('sync_status', 'unknown') for t in not_ready_txns}
                                st.warning(
                                    f"{len(not_ready_txns)} transaction(s) skipped — not marked SYNC_READY in Ramp "
                                    f"(statuses: {', '.join(sorted(not_ready_statuses))}). "
                                    f"Users must mark them ready in the Ramp UI before they can be synced via API."
                                )
                            ...
                            if local_only:
                                ...
                                # Record skipped (not-ready) transactions in results
                                for t in not_ready_txns:
                                    i += 1
                                    tid = t.get('id')
